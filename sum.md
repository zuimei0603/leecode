...
采用字典存储已出现的数字，后续处理时，判断target-curNum是否在字典存在
...
先将数组nums排序，选定起点i，然后使用双指针(l，r)遍历处理，l为i+1，r为数组最后一点。当i,l,r三点之和大于0，则需将r指针左移；
小于0，则将l指针右移。注意重复元素
...
与三数之和为零思路基本一致，都是采用排序+双指针处理。同样是指针l和r左移和右移，无序考虑重复情况。
```
...
AB、CD数组元素相加之和存入字典，然后就和两数之和一样解题思路了
